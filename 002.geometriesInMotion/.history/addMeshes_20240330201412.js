...
export function addPlanet4() {
	const color = tLoader.load('/Planet4/COLOR.jpg')
	const normal = tLoader.load('/Planet4/NORM.jpg')
	const ambientOcclusion = tLoader.load('/Planet4/OCC.jpg')
	const displace = tLoader.load('/Planet4/DISP.png')
	const roughnessMap = tLoader.load('/Planet4/ROUGH.jpg')
	const planetGeometry = new THREE.SphereGeometry(0.3, 200, 100)
	const planetMaterial = new THREE.MeshStandardMaterial({
		map: color,
		normalMap: normal,
	...
	const planetMesh = new THREE.Mesh(planetGeometry, planetMaterial)
	planetMesh.position.set(0, -2, 0)
	return planetMesh
}
export function addPlanet5() {
	const color = tLoader.load('/Planet5/COLOR.png')
	const normal = tLoader.load('/Planet5/NORM.jpg')
	const ambientOcclusion = tLoader.load('/Planet5/OCC.jpg')
	const displace = tLoader.load('/Planet5/DISP.png')
	const roughnessMap = tLoader.load('/Planet5/ROUGH.jpg')
	const planetGeometry = new THREE.SphereGeometry(0.3, 200, 100)
	const planetMaterial = new THREE.MeshStandardMaterial({
		map: color,
		normalMap: normal,
