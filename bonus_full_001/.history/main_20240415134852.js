...
function loader() {
	const loader = document.querySelector('.EnterButton')
	loader.innerHTML = 'Enter'
}
...
function raycasting() {
	window.addEventListener('click', (event) => {
		...
		for (let i = 0; i < intersects.length; i++) {
			...
			if (activeScene.name && activeScene.name !== object.userData.name) {
			...
			if (object.userData.name === activeScene.name) {
				...
				// currentState = { activeModalId: modalId, activeElementId: elementId };
			}
			...
			// 	const targetName = object.userData.name
			// 	moveTarget({ ...targetPosition })
			// 	moveCamera(cameraPosition, targetName)
...
function setActive(targetName) {
	activeScene.text = document.querySelector(`.${targetName}`)
}

function resetActive(targetName) {
	activeScene.text = null
}
