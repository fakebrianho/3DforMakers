...
wheelAdaptor.addEventListener('trigger', () => {
	...
	gsap.to(camera.position, {
		...
		ease: 'back.inOut',
		onComplete: () => {
			// gsap.to(slides[currentItem].rotation, {
			// 	x: '+=3',
			// 	y: '+=3',
			// 	z: '+=3',
			// 	duration: 1,
...
function models() {
	const sword = new Model({
		...
		callback: function callback(slides) {
			console.log(slides)
			// slides.push(this.meshes[`${this.name}`])
		},
	})
	...
	shield.init()
	flowers.init()
	slides.push(meshes.sword)
	slides.push(meshes.shield)
	slides.push(meshes.flowers)
}
